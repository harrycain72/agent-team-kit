...
- When the full check is green, move the card to `functional-test` and assign it to `tester` (never `done`; the lead closes it) and say so in your report. If the tester or perf-tester sent the card back with items under `## Defects`, fix each test-first, tick it, and hand the card back the same way once none is left open.
