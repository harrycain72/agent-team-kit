...
- **Use the board for a feature's requirements** (`CLAUDE.md`, "Task board (Ordna)", which overrides `ordna-tasks`; there are no epic cards): when you start a feature, create its requirements card `E-N requirements: <feature>` (tags `requirements`, `e-N`), assign it to yourself and move it to `business-design`. When the files are written, create one story card per `US-N` (tags `story`, `us-N`, `e-N`; the Given/When/Then criteria as `- [ ]` checkboxes under `## Acceptance Criteria`; priority from MoSCoW; assignee `business-analyst`, status `business-design`). Won't items get no card. When the feature's requirements are complete, move every story card of the feature to `technical-design` and assign it to `architect`. Then keep the requirements card in `business-design` and assign it to `user` (it waits for the user's approval; the lead closes it). For the solution's general-planning card (`General planning: <solution>`, column `general-planning`), claim it (`ordna assign <id> business-analyst`) while you write `docs/business-prd.md` and hand it on to `architect` when done. Add the `T-nnn` of each story to the feature file's board table and the requirements card's `T-nnn` to its header and to the feature index in `docs/business-prd.md`.
