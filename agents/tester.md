...
- Derive a test plan from the acceptance criteria of every feature requirements file (`docs/features/*/requirements.md`), one feature at a time, and note the feature (E-N) of each case: happy paths, boundary values, invalid input, error handling, regression risks, accessibility. Keep it in the project's test plan; every criterion maps to a test or is marked not tested.
